...
**作成日:** 2026-02-09
**ステータス:** In Progress (Task 1, 2, 3, 4, 5 完了)
**推定工数:** 25 時間
...
[2] Image-Aware Card Generation (8h) ✅ 完了 (2026-02-10)
[3] Prompt Evaluation Framework (4h) ✅ 完了 (2026-02-10)
[4] Interactive Review TUI (6h)      ✅ 完了 (2026-02-10)
[5] LinkedIn/GitHub 英語化 (2h)     ✅ 完了 (2026-02-10)
[6] Zenn 紹介記事 1本 (2h)           ← 開発ストラグル焦点の紹介記事
```
...
## Task 2: Image-Aware Card Generation (P0, 8h) ✅ 完了

> **完了日:** 2026-02-10 | **実績:** image.py (252 LOC) + vision.py (198 LOC), 555 tests, 95.16% coverage
> **新規モジュール:** image.py (画像検出・抽出), vision.py (Vision API統合)
> **修正:** schemas.py, extract.py, cost.py, convert.py, config.py, service.py, main.py
...
## Task 3: Prompt Evaluation Framework (P1, 4h) ✅ 完了

> **完了日:** 2026-02-10 | **実績:** eval/ パッケージ (4モジュール, 273 LOC), 586 tests, 92.85% coverage
> **新規モジュール:** eval/dataset.py, eval/matcher.py, eval/metrics.py, eval/report.py
> **修正:** main.py (eval サブコマンド追加)
> **データ:** evals/dataset.yaml (10 cases, 23 expected cards)
...
## Task 4: Interactive Review TUI (P1, 6h) ✅ 完了

> **完了日:** 2026-02-10 | **実績:** tui/ パッケージ (4モジュール, ~198 LOC), 624 tests, 92.26% coverage
> **新規モジュール:** tui/__init__.py, tui/state.py, tui/widgets.py, tui/app.py
> **修正:** main.py (--review フラグ追加), pyproject.toml (textual 依存追加)
...
## Task 5: LinkedIn/GitHub プロフィール英語化 (P2, 2h) ✅ 完了

> **完了日:** 2026-02-10
> **成果物:** README.md/README.ja.md 更新 (3c511cc), GitHub bio 設定, LinkedIn ドラフト作成

### 内容

Phase 1 から移動されたコード外タスク。

- [x] pdf2anki の README.md/README.ja.md を Phase 2 機能反映で更新
- [x] GitHub リポジトリの Description を英語で更新
- [x] GitHub プロフィール bio を英語で設定
- [x] LinkedIn ヘッドライン・概要・職務内容の日英ドラフト作成 (`docs/linkedin-draft.md`)
...
## Task 6: Zenn 紹介記事 1 本公開 (P2, 2h)

### タイトル案

**「Claude API で PDF→Anki カードを自動生成する CLI を作って躓いたこと」**

### コンセプト

プロジェクト紹介 + 開発ストラグルに焦点。機能カタログではなく「何に困って、どう判断したか」を語る。

### 構成案

1. **忘却曲線アプリを作ったら Anki の再発明だった** — g-kentei-ios で出題アプリを自作 → Anki と同じことをしていると気づいた反省。アプリではなくコンテンツ生成を解決すべきだった
2. **pdf2anki: どんなテキストからでも Anki カードを作る** — 動機の転換とデモ（入力→出力の具体例）
3. **LLM 出力は信用できない** — JSON パース失敗、品質バラつき、3層品質保証に至った経緯
4. **コストが見えないと怖い** — API コスト爆発の恐怖、事前見積もり + budget_limit の設計
5. **長文 PDF をどう分割するか** — トークン制限との戦い、セクション分割 + breadcrumb の試行錯誤
6. **OpenAI 対応を捨てた話** — Root Cause Challenge で ROI 評価 → 1,000行書いて$10節約は割に合わない
7. **画像カードという沼** — Vision API のトークンコスト、DPI とリサイズの妥協点
8. **プロンプトの品質をどう測るか** — Eval Framework を作った動機、キーワードマッチの限界
9. **まとめ** — 依存 8 個で済ませる設計思想、今後

### 執筆方針

- 各セクションは「問題 → 試行 → 判断」の構造
- コード例は最小限（設計判断の説明に必要な分だけ）
- 読者ターゲット: LLM を使ったツールを作りたい開発者
...
- [x] Vision API で画像を含むカードが生成できる (Task 2, 2026-02-10)
- [x] TUI でカードを対話的にレビューできる (Task 4, 2026-02-10)
- [x] プロンプト評価フレームワークが動作する (Task 3, 2026-02-10)
- [x] テキスト抽出がキャッシュで高速化されている (Task 1, 2026-02-10)
- [ ] Zenn に紹介記事 1 本が公開されている
- [x] テストカバレッジ 80% 以上を維持 (92.26%)
- [x] ローカルフォルダ名を `Anki-QA` → `pdf2anki` にリネーム + Claude Code メモリ移行 (2026-02-10)
- [x] v0.3.0 リリース (2026-02-10)
